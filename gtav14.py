...
class YogaSystem:
    ...
    def check_smart_mirror_logic(self, pose_name, profile):
        ...
        if "vrikshasana" in pose or "tree" in pose:
            l_knee = profile['Left Knee']
            r_knee = profile['Right Knee']
    ...
    def is_valid_variation(self, pose_name, joint, user_angle, ideal_angle):
        """Forgives advanced versions of poses that the AI might think are 'too bent'."""
        pose = pose_name.lower()
        
        # --- PADAMASANA (LOTUS POSE) ADVANCED LOGIC ---
        if "padamasana" in pose or "lotus" in pose:
            # In Lotus, a tighter knee/hip bend is BETTER. 
            # If the user's angle is deeply bent (< 60), or tighter than the ideal, forgive it.
            if "Knee" in joint:
                if user_angle < 60: return True  # Deep full lotus
                if user_angle < ideal_angle: return True # Better than the dataset average
            
            if "Hip" in joint:
                if user_angle < 110: return True # Deep seated posture
                
            # Arms resting on knees (elbows straight or slightly bent)
            if "Elbow" in joint:
            ...
            for joint, user_angle in profile.items():
                ...
                if joint in refs:
                    ...
                    if diff > STRICT_TOLERANCE:
                        # --- Check for Advanced Variations (like tight Padmasana) ---
...
if __name__ == "__main__":
    print("=== YOGA AI (WITH ADVANCED POSE VARIATIONS) ===")
