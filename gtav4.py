...
class GeometryEngine:
    ...
    def get_pose_profile(landmarks):
        ...
        This is the 'Fingerprint' of the pose.
        """
        frame = landmarks
        profile = {}
        ...
        # 2. Armpit Angles (Arm vs Torso) - Crucial for "Arms Up" detection
        # We assume Shoulder(11) -> Elbow(13) vs Shoulder(11) -> Hip(23)
        # This measures how high the arm is raised relative to the body.
...
# ==========================================
# PART 3: UNIVERSAL ANGLE LEARNER
# ==========================================
def learn_ideal_poses_only():
    """Scans dataset and updates the 'Gold Standard' file."""
    if not os.path.exists(PROCESSED_DATA_FILE):
        print("Error: No processed data found. Run Option 1 first to process videos.")
        return
    ...
    print(f"Scanning {len(train_data)} samples to learn ideal angles...")
    
    # Storage: { 'TreePose': { 'Left Knee': [180, 179...], 'Left Armpit': [170, 175...] } }
    stats = {cls: {} for cls in class_map}

    for i in tqdm(range(len(train_data))):
        pose_name = idx_to_class[train_labels[i]]
        frame = train_data[i][SEQ_LENGTH // 2] # Middle frame
    ...
    print(f"Success! Updated {LEARNED_ANGLES_FILE} with new Armpit data.")
...
class YogaSystem:
    def __init__(self):
        ...
        if os.path.exists(LEARNED_ANGLES_FILE):
            ...
            print("WARNING: No ideal angles found. Run Option 2!")
    ...
    def analyze_user_input(self, input_path):
        ...
        feedback = []
        user_profile = GeometryEngine.get_pose_profile(raw_data[-1]) # Get user's angle profile
        
        if pred_class in self.reference_angles:
            refs = self.reference_angles[pred_class]
            
            # Loop through EVERY joint we track (Elbows, Knees, ARMPITS)
            for joint, user_angle in user_profile.items():
                if joint in refs:
                    target, std = refs[joint]
                    
                    # Universal Threshold: 2 standard deviations + 15 degrees buffer
                    threshold = (std * 2) + 15
                    
                    if abs(user_angle - target) > threshold:
                        # Generic corrections based on direction
                        if "Armpit" in joint:
                            action = "Raise" if user_angle < target else "Lower"
                            joint_name = joint.replace("Armpit", "Arm") # UI friendly name
                        else:
...
if __name__ == "__main__":
    ...
    print("1. Full Training (Slow)")
    print("2. Learn Ideal Angles ONLY (Fast - Run this to update logic!)")
    print("3. Predict")
    
    choice = input("Select: ")
    if choice == '1': 
        # (Include full training pipeline here if needed, or user calls Part 2 logic)
        print("Please run data preprocessing/training steps from previous code.")
    elif choice == '2':
        learn_ideal_poses_only()
    elif choice == '3':
