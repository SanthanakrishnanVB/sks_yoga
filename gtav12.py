...
class YogaSystem:
    ...
    def check_smart_mirror_logic(self, pose_name, profile):
        """
        Handles asymmetric poses (like Tree Pose).
        Checks if the USER is correct based on logic, ignoring the biased dataset.
        Returns: (Is_Correct, List_of_Corrections_to_IGNORE)
        """
        ...
        if "vrikshasana" in pose or "tree" in pose:
            # 1. LEG LOGIC: One straight, one bent
            left_knee = profile['Left Knee']
            right_knee = profile['Right Knee']
            
            # Check Symmetry: One leg > 150 (Straight) AND One leg < 110 (Bent)
            if (left_knee > 150 and right_knee < 110) or (right_knee > 150 and left_knee < 110):
                # Legs are CORRECT regardless of side!
                ignore_joints.append("Left Knee")
                ignore_joints.append("Right Knee")
                ignore_joints.append("Left Hip")
                ignore_joints.append("Right Hip")
                is_correct = True
            
            # 2. ARM LOGIC: Arms can be Chest (40-100) or Overhead (>130)
            # We don't check symmetry here, usually both arms do the same thing
            l_arm = profile['Left Armpit']
            r_arm = profile['Right Armpit']
            
            # If arms are high enough OR in prayer position, ignore arm errors
            if (l_arm > 130 and r_arm > 130) or (30 < l_arm < 100 and 30 < r_arm < 100):
                ignore_joints.append("Left Armpit")
                ignore_joints.append("Right Armpit")
                ignore_joints.append("Left Elbow")
                ignore_joints.append("Right Elbow")
                ignore_joints.append("Left Shoulder")
                ignore_joints.append("Right Shoulder")
    ...
    def analyze_user_input(self, input_path):
        ...
        # Override Tadasana if knees are asymmetric (Tree Pose)
        if (left_knee < 110 or right_knee < 110) and "Tadasana" in pred_class:
            print(f"Override: Detected Tree Pose geometry.")
        ...
        # Check for smart variations (Mirror Logic)
        ...
        if pred_class in self.reference_angles:
            ...
            for joint, user_angle in profile.items():
                # Skip if "Smart Logic" says this joint is okay
...
if __name__ == "__main__":
    print("=== YOGA AI (WITH MIRROR LOGIC) ===")
