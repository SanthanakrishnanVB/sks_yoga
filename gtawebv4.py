...
class YogaSystem:
    ...
    def is_valid_variation(self, pose_name, joint, user_angle, ideal_angle):
        ...
        # --- TADASANA (MOUNTAIN POSE) V5 LOGIC ---
        if "tadasan" in pose or "mountain" in pose:
            # Overrides camera depth issues for elbows when arms are overhead/clasped
            if "Elbow" in joint and user_angle > 110: 
                return True
            if "Knee" in joint and user_angle > 140: 
                return True

        # --- VRIKSHASANA (TREE POSE) LOGIC ---
        ...
        # --- PADMASANA (LOTUS POSE) LOGIC ---
        if "padma" in pose or "padam" in pose or "lotus" in pose:
            if abs(user_angle - ideal_angle) <= 30: return True
    ...
    # ---------------------------------------------------------
    # STATIC IMAGE DISPLAY (LIGHT MODE UI)
    # ---------------------------------------------------------
    def show_results(self, img, cls, conf, feedback):
        ...
        sidebar_w = 600
        # Light Mode: White background (255, 255, 255)
    ...
    # ---------------------------------------------------------
    # LIVE WEBCAM DISPLAY (LIGHT MODE UI)
    # ---------------------------------------------------------
    def show_live_results(self, img, cls, conf, feedback):
        ...
        # Header (Black text)
        cv2.putText(sidebar, f"Pose: {cls}", (15, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 2)
        # Subheader (Dark Grey text)
        cv2.putText(sidebar, f"Conf: {conf:.1f}%", (15, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (80, 80, 80), 1)
        ...
        if not feedback or feedback[0] == "Perfect Posture!" or feedback[0] == "Please step into the frame":
            ...
            cv2.putText(sidebar, msg, (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
        else:
            # Alert Header (Orange)
            cv2.putText(sidebar, "Corrections Needed:", (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 100, 255), 2)
            y += 35
            for line in feedback:
                if y > h - 15:
                    cv2.putText(sidebar, "... and more", (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 100, 100), 1)
                    break
                # Normal feedback text (Black)
                cv2.putText(sidebar, f"- {line}", (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 0), 1)
                y += 25
        ...
        screen_max_h = 700
        if h > screen_max_h:
            scale = screen_max_h / h
            final_output = cv2.resize(final_output, (int((w + sidebar_w) * scale), screen_max_h))
            
        cv2.imshow("Yoga AI Live Tracking (Press 'q' to quit)", final_output)

if __name__ == "__main__":
    print("=== YOGA AI (V5 - FOOLPROOF LOGIC) ===")
