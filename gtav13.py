...
class YogaSystem:
    ...
    def check_smart_mirror_logic(self, pose_name, profile):
        """
        Detects if the user is doing an asymmetric pose correctly, 
        regardless of which side they are using.
        """
        ...
        if "vrikshasana" in pose or "tree" in pose:
            ...
            # --- LEG LOGIC ---
            # Is there a clear difference between the legs? (One straight, one bent)
            if abs(l_knee - r_knee) > 30: 
                standing_leg = max(l_knee, r_knee) # The straighter one
                bent_leg = min(l_knee, r_knee)     # The bent one
                
                # Broad Check: 
                # Standing leg > 100 (allows for some camera tilt/noise)
                # Bent leg < 130 (Tree pose is usually < 90, but loose tree is OK)
            ...
            # Allow Hands Overhead (>130) OR Hands at Chest (30-100)
    ...
    def analyze_user_input(self, input_path):
        ...
        # Force Vrikshasana if one knee is bent (Tadasana requires both straight)
        if (l_knee < 110 or r_knee < 110) and "Tadasana" in pred_class:
        ...
        # Get joints to IGNORE because they are actually correct variations
        ...
        if pred_class in self.reference_angles:
            ...
            for joint, user_angle in profile.items():
                if joint in ignore_joints: continue # Skip ignored joints
