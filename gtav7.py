...
# === STRICTNESS SETTINGS ===
STRICT_TOLERANCE = 12.0  # Max allowed error in degrees (Lower = Stricter)
SEQ_LENGTH = 30
NUM_NODES = 33
CHANNELS = 3
BATCH_SIZE = 16
...
class GeometryEngine:
    ...
    def get_pose_profile(landmarks):
        ...
        # Armpit Angles (Crucial for Vrikshasana/T-Pose detection)
...
def learn_ideal_poses_logic(datasets, class_map):
    ...
    for cls in stats:
        ...
        for joint, angles in stats[cls].items():
            if len(angles) > 0:
                final_refs[cls][joint] = np.mean(angles) # ONLY MEAN, NO STD DEV
...
# ==========================================
# PART 4: SYSTEM & STRICT PREDICTION
# ==========================================
class YogaSystem:
    def __init__(self):
    ...
    def analyze_user_input(self, input_path):
        ...
        if pred_class in self.reference_angles:
            ...
            for joint, user_angle in user_profile.items():
                if joint in refs:
                    target = refs[joint] # Just the mean
    ...
    def show_results(self, img, cls, conf, feedback):
        h, w, _ = img.shape
        cv2.rectangle(img, (0, 0), (450, h), (30, 30, 30), -1)
        ...
        try: cv2.imshow("Yoga AI Strict", img); cv2.waitKey(0); cv2.destroyAllWindows()
        except: pass

if __name__ == "__main__":
    print("=== STRICT YOGA AI ===")
    print("1. Full Training")
    print("2. Update Ideal Angles")
    print("3. Predict (Strict Mode)")
