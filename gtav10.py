...
class YogaSystem:
    ...
    def is_valid_variation(self, pose_name, joint, user_angle, ideal_angle):
        """
        Handles Yoga poses that have multiple correct ways to perform them.
        Returns True if the user's angle is an acceptable variation.
        """
        pose = pose_name.lower()
        
        if "vrikshasana" in pose or "tree" in pose:
            # 1. Arm Variations: Accept hands at chest (Ideal ~60) OR hands overhead (~150-180)
            if "Arm" in joint or "Shoulder" in joint:
                if user_angle >= 140: # User has hands above head
                    return True
                if 40 <= user_angle <= 90: # User has hands at chest
                    return True
                    
            # 2. Leg Variations: The bent knee can be placed high (tight angle) or low (loose angle)
            if "Knee" in joint or "Hip" in joint:
                if user_angle < 90 and ideal_angle < 90: 
                    return True 
                    
            # 3. Elbow Variations: Arms overhead often bend slightly
            if "Elbow" in joint:
                if user_angle > 130: 
                    return True
    ...
    def analyze_user_input(self, input_path):
        ...
        if is_tree_pose_geometry and "Tadasana" in pred_class:
            ...
            for joint, user_angle in profile.items():
                if joint in refs:
                    ...
                    if diff > STRICT_TOLERANCE:
                        # --- Check for valid variations before penalizing ---
                        ...
                        feedback.append(f"{action} {joint} (You:{formatted_u}°, Ideal:{formatted_t}°)")
