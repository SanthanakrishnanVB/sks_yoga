...
# === STRICTNESS SETTINGS ===
STRICT_TOLERANCE = 15.0  # Default allowed error for most standing poses
SEQ_LENGTH = 30
NUM_NODES = 33
CHANNELS = 3
BATCH_SIZE = 16
...
class YogaSystem:
    ...
    def check_smart_mirror_logic(self, pose_name, profile):
        ...
        if "vrikshasana" in pose or "tree" in pose:
            ...
            # Variation A: Hands Overhead (Armpits high, elbows mostly straight)
            overhead = (l_arm > 130 and r_arm > 130) and (l_elbow > 130 and r_elbow > 130)
            
            # Variation B: Hands in Namaste at Chest (Armpits low, elbows MUST be bent)
    ...
    def is_valid_variation(self, pose_name, joint, user_angle, ideal_angle):
        ...
        if "vrik" in pose or "tree" in pose:
            ...
            # We removed the "ignore all straight elbows" rule here!
        return False
    ...
    # ---------------------------------------------------------
    # NEW: LIVE WEBCAM PROCESSING LOOP
    # ---------------------------------------------------------
    def analyze_webcam(self):
        """Opens webcam and predicts yoga poses lively frame-by-frame."""
        print("\n" + "="*50)
        print("🎥 LIVE WEBCAM MODE STARTED")
        print("Please step back so your full body is in the frame.")
        print("Press 'q' or 'ESC' on your keyboard to stop.")
        print("="*50 + "\n")

        cap = cv2.VideoCapture(0)
        
        if not cap.isOpened():
            print("❌ CRITICAL ERROR: Could not open webcam at index 0.")
            return
    ...
    def _process_frame_logic(self, img, return_landmarks=False):
        ...
        left_knee = profile.get('Left Knee', 180)
        right_knee = profile.get('Right Knee', 180)
        
        if (left_knee < 110 or right_knee < 110) and "Tadasana" in pred_class:
        ...
        # ========================================================
        # DYNAMIC TOLERANCE: Changes strictness based on the pose!
        # ========================================================
        pose_lower = pred_class.lower()
        current_tolerance = STRICT_TOLERANCE
        
        # If it catches anything spelled like Padma/Lotus, give it a HUGE 60-degree error allowance!
        if "padam" in pose_lower or "padma" in pose_lower or "lotus" in pose_lower:
            current_tolerance = 25.0 
        # ========================================================
        ...
        if pred_class in self.reference_angles:
            ...
            for joint, user_angle in profile.items():
                ...
                if joint in refs:
                    ...
                    # Checks the difference against the DYNAMIC tolerance (15 for standing, 60 for Padmasana)
                    if diff > current_tolerance:
                        
                        # Extra rule for Padmasana: If you bend deeper than the ideal, it's ALWAYS perfect.
                        if ("padma" in pose_lower or "padam" in pose_lower) and ("Knee" in joint or "Hip" in joint):
                            if user_angle < target: 
                                continue # Forgive it, deeper is better!
    ...
    def show_live_results(self, img, cls, conf, feedback):
        h, w, _ = img.shape
        sidebar_w = 450
        sidebar = np.ones((h, sidebar_w, 3), dtype=np.uint8) * 35 
        
        cv2.putText(sidebar, f"Pose: {cls}", (15, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2)
        cv2.putText(sidebar, f"Conf: {conf:.1f}%", (15, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 1)
        
        y = 130
        if not feedback or feedback[0] == "Perfect Posture!" or feedback[0] == "Please step into the frame":
            color = (0, 255, 0) if feedback[0] == "Perfect Posture!" else (0, 165, 255)
            cv2.putText(sidebar, feedback[0], (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
        else:
            cv2.putText(sidebar, "Corrections Needed:", (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 165, 255), 2)
            y += 35
            for line in feedback:
                if y > h - 15:
                    cv2.putText(sidebar, "... and more", (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                    break
                color = (0, 0, 255) if any(x in line for x in ["Straighten", "Bend", "Raise", "Lower"]) else (0, 255, 0)
                cv2.putText(sidebar, f"- {line}", (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 1)
                y += 25
                
        final_output = cv2.hconcat([sidebar, img])
        cv2.imshow("Yoga AI Live Dashboard (Press 'q' to quit)", final_output)


if __name__ == "__main__":
    # Check this exact line when running to ensure you have V4!
    print("=== YOGA AI (DYNAMIC TOLERANCE V4) ===") 
