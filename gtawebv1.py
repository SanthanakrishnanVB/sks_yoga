...
# === STRICTNESS SETTINGS ===
STRICT_TOLERANCE = 15.0  # Default allowed error for most poses
SEQ_LENGTH = 30
NUM_NODES = 33
CHANNELS = 3
BATCH_SIZE = 16
...
class YogaSystem:
    ...
    def check_smart_mirror_logic(self, pose_name, profile):
        ...
        if "vrikshasana" in pose or "tree" in pose:
            ...
            if (l_arm > 130 and r_arm > 130) or (30 < l_arm < 100 and 30 < r_arm < 100):
    ...
    def is_valid_variation(self, pose_name, joint, user_angle, ideal_angle):
        ...
        # --- PADMASANA (LOTUS POSE) 30-DEGREE THRESHOLD ---
        if "padma" in pose or "lotus" in pose or "Padamasana" in pose:
            # 1. Provide a flat 30 degree error tolerance for ALL joints in Padmasana
            if abs(user_angle - ideal_angle) <= 60:
                return True
                
            # 2. Keep the 'deeper bend is better' rule for knees and hips
    ...
    def analyze_webcam(self):
        ...
        if not cap.isOpened():
            ...
            print("Check if another app is using the camera or change the index in the code.")
            return
            
        # Set resolution for stable performance
        ...
        try:
            while cap.isOpened():
                ...
                # Process the live frame to get feedback
                ...
                # Press 'q' or ESC to quit the webcam loop
    ...
    def show_live_results(self, img, cls, conf, feedback):
        """Builds a compact sidebar suitable for live video feed."""
        h, w, _ = img.shape
        sidebar_w = 450
        ...
        if not feedback or feedback[0] == "Perfect Posture!" or feedback[0] == "Please step into the frame":
            ...
            y += 35
            for line in feedback:
                # Prevent text going off the bottom of the screen
                if y > h - 15:
...
if __name__ == "__main__":
    print("=== YOGA AI (LIVE WEBCAM ENABLED) ===")
